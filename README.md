...
7. On the left-side panel, click the Build dropdown and select Authentication.
   a. Add Email/Password and Google as authentication methods.
   b. Click the Build dropdown again and select Firebase Database.
   c. Create a new Cloud Firestore database with a collection titled "users."
9. To install CocoaPods dependencies, open the Terminal and navigate to the folder containing the cloned repository using ``cd`` and the file's entire pathway.
